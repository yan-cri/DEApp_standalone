...
edgeR.de.analysis <- function(countTabpath, metaTabpath, cpmCutoff, sampCutoff, group1, group2, padjust, Pcutoff, FCcutoff, GeneRegFilter, saveRes) {
  ...
  metaTab <- read.delim(as.character(metaTabpath), header = T, check.names=FALSE)
  ...
  #dge.count.rmlow2 <- DGEList(counts=dge.count.rmlow$counts, group=Group)
  #dge.count.rmlow2 <- calcNormFactors(dge.count.rmlow2)
  
  log.cpm.rmlow <- cpm(dge.count.rmlow, normalized.lib.sizes=T, log=T)
  cpm.rmlow <- cpm(dge.count.rmlow, normalized.lib.sizes=T, log=F)
  if (saveRes==TRUE) {
    edgeR.res.path <- paste(getwd(), "/edgeR-res", sep="")
  ...
  test <- glmLRT(glm.fit) ##by default the test is for the last coefficient in the design matrix
  
  tpsave <- topTags(test,n=Inf, adjust.method="BH", sort.by="PValue")
  #cpm(dge.count.rmlow$counts)[rownames(tpsave),]
  #cpm.rmlow[rownames(tpsave),]
...
limmavoom.de.analysis <- function(countTabpath, metaTabpath, cpmCutoff, sampCutoff, group1, group2, padjust, Pcutoff, FCcutoff, GeneRegFilter, saveRes) {
  ...
  if (saveRes==TRUE) {
    limmavoom.res.path <- paste(getwd(), "/limma-voom-res", sep="")
  ...
  contrast.fit <- contrasts.fit(fit, coefficients = dim(design)[2])
...
deseq2.de.analysis <- function(countTabpath, metaTabpath, cpmCutoff, sampCutoff, group1, group2, padjust, Pcutoff, FCcutoff, GeneRegFilter, saveRes) {
  ...
  if (saveRes==TRUE) {
    deseq2.res.path <- paste(getwd(), "/DEseq2-res", sep="")
  ...
  dds <- DESeqDataSetFromMatrix(countData=as.matrix(dge.count.rmlow$count), colData=DataFrame(Batch, Group), design=formula(~Batch+Group) )
